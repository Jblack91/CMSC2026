...
public class CircusDriverApp_GUI extends Application {
    ...
    private void handleGenerateTicket() {
        ...
        while (moreTickets) {
            ...
            if (!dayOpt.isPresent()) break;

            String selectedDay = dayOpt.get();
            String day = selectedDay.split(" ")[0];

            List<String> customerChoices = new ArrayList<String>();
            customerChoices.add("Child (10% discount)");
            customerChoices.add("Student (10% discount)");
            customerChoices.add("Adult");
            customerChoices.add("Senior (5% discount)");
            ...
            {
                case "Child (10% discount)":
                case "Student (10% discount)":
                    customerDiscount = 0.10;
                    break;

                case "Senior (5% discount)":
            ...
            List<String> seatChoices = new ArrayList<String>();
            seatChoices.add("Lower Level");
            seatChoices.add("T-Level");
            seatChoices.add("Upper Level");
 ...
 // ===================== DISPLAY ANIMALS WITH IMAGES =====================
    ...
    private String getSingleTicketDetails(String dayOfWeek, double basePrice, String seatLevel, double customerDiscount) {
        ...
        switch (seatLevel)
        {
            case "T-Level":
            {
                double oldPrice1 = price;
                price *= 2;
                sb.append(String.format("T-Level Seat double price: $%.2f -> $%.2f%n", oldPrice1, price));
                break;
            }

            case "Upper Level":
            {
                double oldPrice2 = price;
                price *= 1.5;
                sb.append(String.format("Upper Level Seat 50%% extra: $%.2f -> $%.2f%n", oldPrice2, price));
                break;
            }

            default:
            {
                sb.append("Lower Level Seat: No extra charge\n");
                break;
            }
        }
